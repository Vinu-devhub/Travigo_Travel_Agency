  ...
  return (
    ...
    <div className='p-5 rounded-lg bg-slate-50/5 ring-1 ring-slate-100 shadow-md shadow-slate-200 hover:scale-105 duration-300 transition-all cursor-pointer'>
        ...
        <div className='grid items-center gap-3 px-5'>
            {
                plancontent?.map((item, index) => (
                    <div key={index} className="flex items-center justify-start gap-5">
                        <div className='grid items-center'>
                            <img src={item.iconbox} alt="iconbox/svg" className='lg:w-5 lg:h-5' />
                        </div>
                        <div className='grid items-center'>
                            <h1 className='font-normal text-lg text-gray-900 filter drop-shadow-lg lg:text-base hover:text-emerald-500'>{item.text}</h1>
                        </div>
                    </div>
                ))
            }
        ...
        <div className='grid items-center justify-items-center mt-7'>
            <button type='button' className='button-solid text-base'>{buttonText}</button>
        </div>
    </div>
    </>
  )
